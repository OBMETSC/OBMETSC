...
def output_power_production(input_technology, power_input, location):
    input_technology = str(input_technology)
    power_input = int(power_input)
    ...
    if input_technology == "Wind+PV":#if Wind+PV is chosen you will get a 50/50 plant
        production_pv = 0.5 * power_input * pd.DataFrame(dict_ort[location][0], columns=['electricity'])
        production_wind = 0.5 * power_input * pd.DataFrame(dict_ort[location][1], columns=['electricity'])
...
def dcf_power_production(input_technology, power_input, capex_power, opex_power, runtime, location, power_cost, wacc, price_change):
    list1 = list(range(0, 8760))
    list1[0:8760] = [int(0) for i in list1[0:8760]]
    ...
    output_pp = output_power_production(input_technology, power_input, location)
...
def output_power_to_x(power_technology, input_technology, efficiency, product_price, margincost_model, variable_cost, location, power_input, price_change):
    list1 = list(range(0, 8760))
    list2 = list1.copy()
    ...
    if input_technology != "Grid":
        output_pv = output_power_production(input_technology, power_input, location)['pv_production']
        output_wind = output_power_production(input_technology, power_input, location)['wind_production']
...
def dcf_power_to_x(power_technology, capex_technology, opex_technology, runtime, power_cost,
    ...
    if input_technology != "Grid":
        dcf_power = dcf_power_production(input_technology, power_input, capex_power, opex_power, runtime,
                                location, power_cost, wacc, price_change)[0]
...
def infrastructure_dimension(ptx_technology, infrastructure_type, distance, power_technology, input_technology,
    ...
    if infrastructure_type == "Pipeline":
        ...
        pipe_diameter = math.sqrt((4 / math.pi) * (gas_flow / (20 / 3.6)))  #formula for calculation of the pipe diameter (20/3.6 entspricht 20 kmh in m/s)
    ...
    return (amount_trailer, storage_dimension, amount_storage, transport_pressure, pipe_diameter, pipe_length, throughput)
...
def infrastructure_dcf(ptx_technology, infrastructure_type, distance, power_technology, input_technology,
                       ...
                       capacity, opex_trailer, capex_liquifier, opex_liquifier_rate):
    ...
    infrastructure_dcf = pd.DataFrame({"year": list3, "expenditure_transport": list4, "expenditure_compressor": list5, "expenditure_storage": list6})
    infrastructure_dcf['expenditure_total'] = infrastructure_dcf["expenditure_transport"] + infrastructure_dcf["expenditure_compressor"] + infrastructure_dcf["expenditure_storage"]
    x = 0
    npv_calc = list3
